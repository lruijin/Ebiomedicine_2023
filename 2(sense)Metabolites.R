...
for (visit in c(0, 1, 2, 4)) {    # By visit
    ...
    DF <- data.frame(ID = DF$ID, pairID = DF$PairID, v = (1/DF$wt)^2, GA_blood = DF$GA_blood,
                     ...
                     anthro = DF$bw.x, alcohol = DF$Alcohol_bin, GWG = DF[,paste0("GWGv",visit)], 
                     batch = factor(DF$batch), post = DF$measuredate, DF[,2:421])
    met_index <- c(16:435)
    ...
    lme_met <- function(x) {
        ...
        model = lme(anthro ~ met + age + race + education + preBMI + GA_blood + 
                        Gender + nulli + alcohol + post + GWG, 
                    data = nDF2, random = ~1 | pairID, weights = varFixed(~v))
