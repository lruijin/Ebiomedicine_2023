...
for (visit in c(0, 1, 2, 4)) {    # By visit
  ...
  lme_met <- function(x) {
    ...
    DF2$met <- x
    DF2 <- na.omit(DF2)
    model = lmer(anthro ~ met + age + race + education + preBMI + GA_blood + Gender + nulli + (1 | pairID), 
...
for(j in 1:length(anthros)){
  ...
  write.csv(res,file = paste0("Result/Table 2/individual analysis_continuous/table2_",anthros[j],"_long.csv"), row.names = F)
  write.csv(tab.res, file = paste0("Result/Table 2/individual analysis_continuous/table2_",anthros[j],".csv"), row.names = F)
  write.csv(res_merge, file = paste0("Result/Table 2/individual analysis_continuous/table2_",anthros[j],"_complete.csv"), row.names = F)
  
}

####--------------------- plot the overall beta plots for 4 visits ------------------- ####
res <- read.csv("Result/Table 2/individual analysis_continuous/bw/table2_bw_long.csv")
...
mycolors12 = c(brewer.pal(name="Dark2", n = 6), brewer.pal(name="Paired", n = 6))
  ...
  for(k in c("bw","bwz","head_circ","ab_circ","length","ssf")){
    res <- read.csv(paste0("Result/Table 2/individual analysis_continuous/",k,"/table2_",k,"_long.csv"))
