...
anthro_gwg <- read_sas("../Data/formsingle.sas7bdat", 
                   col_select=c("SUBJECT_ID","Weightv0","Weightv1","Weightv2","Weightv3",
                                "Weightv4")) %>%
    rename(ID = SUBJECT_ID) %>% 
    mutate(ID = as.integer(ID))
anthro_gwg <- unique(anthro_gwg %>% mutate(GWG = Weightv4-Weightv0) %>%
                         dplyr::select(ID,GWG)) %>% filter(ID %in% lipid$ID)
...
anthro <- anthro %>% left_join(anthro_gwg, by = "ID")
...
for(i in met_index){
    num_groups[i-22] <- length(levels(anthro[,i]))
}
table(num_groups)
...
for(idx in anthros_idx){
    ...
    lme_met <- function(x) {
        ...
        model = lmer(outcome ~ met + age + race + education + preBMI + Gender + GWG + nulli+ (1 | pairID), 
...
for(ctt in 1:6){
    ...
    lme_met <- function(x) {
        ...
        #model = lme(anthro ~ met + age + as.factor(race) + preBMI + parity + female + GADel + glucose, 
        #            data = nDF2, random = ~1 | pairID, weights = varFixed(~v))
        model = lmer(outcome ~ met + age + race + education + preBMI + GWG + Gender + nulli+ (1 | pairID), 
                     data = nDF2, weights = wt)
        postHocs <- glht(model, linfct = mcp("met" = "Tukey"))
        est <- c(Anova(model,type = 2)[[3]][1], summary(postHocs)$test$coefficients,
                 summary(postHocs)$test$sigma,
...
ggplot(F_pval, aes(x = Annotation, y = -log10(pFDR))) + 
    ...
    scale_color_manual(name="Lipid Class",
                       values= mycolors12)
ggsave("../Figures/revision/trajectory analysis/addGWG.png")
